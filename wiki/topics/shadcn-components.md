...
실제 앱에서는 `src/components/shadcn/Button.tsx`가 더 정교하게 커스터마이징됐고, 내부에서는 `src/components/Button.tsx`(tailwind-variants 기반)와 다른 별도 구현이 공존한다.

## Component Inventory [coverage: high -- 10 sources]
...
- **cva vs tailwind-variants**: shadcn 컴포넌트는 `cva`(class-variance-authority) 사용, 일반 컴포넌트는 `tailwind-variants`(tv) 사용. 두 라이브러리 공존.
